/**
 * Nyx Config API — read/write ~/.nyxhive/config.yaml and ~/.nyxhive/.env.
 * Route name is kept for compatibility with the imported workspace shell.
 */
...
const CONFIG_PATH = path.join(NYX_HOME, 'config.yaml')
const ENV_PATH = path.join(NYX_HOME, '.env')

// Known Nyx workspace providers
const PROVIDERS = [
...
function writeConfig(config: Record<string, unknown>): void {
  fs.mkdirSync(NYX_HOME, { recursive: true })
...
function checkAuthStore(providerId: string): {
  ...
  // Check Nyx auth store first, then legacy imported workspace stores.
  for (const storePath of [
    path.join(NYX_HOME, 'auth-profiles.json'),
    ...
    try {
      ...
      for (const [key, value] of Object.entries(profiles)) {
        ...
        if (token) {
          const source = storePath.includes('.nyxhive')
            ? 'nyx-auth-store'
            : storePath.includes('.hermes')
              ? 'legacy-auth-store'
              : 'openclaw-auth-store'
...
export const Route = createFileRoute('/api/nyx-config')({
  server: {
    handlers: {
      GET: async ({ request }) => {
      ...
      PATCH: async ({ request }) => {
        ...
        return Response.json({
          ok: true,
          message: 'Config updated. Restart Nyx to apply changes.',
        })
      },
    },
  },
