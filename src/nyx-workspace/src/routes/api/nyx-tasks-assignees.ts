 ...
 * Reads agent profiles from the Nyx gateway and combines with the
...
const PROFILES_PATH = path.join(NYX_HOME, 'profiles')
...
export const Route = createFileRoute('/api/nyx-tasks-assignees')({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          const res = await fetch(`${NYX_API_URL}/api/tasks/assignees`, {
