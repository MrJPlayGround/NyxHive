  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          // Step 1: Get session data from Nyx
          let sessionData: Record<string, unknown> | null = null

          if (sessionId) {
            try {
              const res = await fetch(
                `${NYX_API_URL}/api/sessions/${encodeURIComponent(sessionId)}`,
                {
                  headers: authHeaders(),
          ...
          if (!sessionData) {
            try {
              const listRes = await fetch(
                `${NYX_API_URL}/api/sessions?limit=1`,
                {
                  headers: authHeaders(),
                  signal: AbortSignal.timeout(3000),
                },
            ...
            try {
              ...
              if (targetSessionId) {
                const msgRes = await fetch(
                  `${NYX_API_URL}/api/sessions/${encodeURIComponent(targetSessionId)}/messages`,
                  {
                    headers: authHeaders(),
