...
} from '@/screens/settings/settings-architecture'
...
type SettingsPageSectionId = Exclude<SettingsSectionId, 'quick' | 'mcp'>

const SETTINGS_NAV_GROUPS = SETTINGS_GROUPS.map((group) => ({
  ...group,
  sections: group.sections.filter((section) => section.id !== 'quick'),
}))
...
function SettingsRoute() {
  ...
  const [activeSection, setActiveSection] =
    useState<SettingsPageSectionId>('providers')
      ...
      <main className="relative mx-auto flex w-full max-w-5xl flex-col gap-4 px-4 pt-6 pb-24 sm:px-6 md:flex-row md:gap-6 md:pb-8 lg:pt-8">
            ...
            <div className="flex flex-col gap-3">
              {SETTINGS_NAV_GROUPS.map((group) => (
                <div key={group.id} className="space-y-0.5">
                  <p className="px-3 pb-1 text-[11px] font-semibold uppercase tracking-wide text-primary-500">
                    {group.label}
                  </p>
                  {group.sections.map((item) =>
                    item.to ? (
                      <Link
                        key={item.id}
                        to={item.to}
                        className="block rounded-lg px-3 py-2 text-left text-sm text-primary-600 transition-colors hover:bg-primary-100 hover:text-primary-900"
                      >
                        {item.label}
                      </Link>
                    ) : (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setActiveSection(item.id as SettingsPageSectionId)
                        }
                        className={cn(
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          ...
          {activeSection === 'providers' && (
            <NyxConfigSection activeView="providers" />
          )}
          {activeSection === 'agent' && (
            <NyxConfigSection activeView="agent" />
          )}
          {activeSection === 'routing' && (
            <NyxConfigSection activeView="routing" />
          )}
          {activeSection === 'voice' && (
            <NyxConfigSection activeView="voice" />
          )}
          {activeSection === 'memory' && (
            <NyxConfigSection activeView="memory" />
          )}
          {activeSection === 'agent-display' && (
            <NyxConfigSection activeView="display" />
          )}
          {(activeSection === 'backend-status' ||
            activeSection === 'config-sources') && (
            <NyxConfigSection activeView="advanced" />
          )}
          ...
          {activeSection === 'appearance' && (
            <>
              ...
              {/* LoaderStyleSection removed — not relevant for Nyx */}
            </>
          )}
          ...
          {/* ── Editor ──────────────────────────────────────────── */}
          {activeSection === ('editor' as SettingsPageSectionId) && (
            <SettingsSection
              title="Editor"
          ...
          <footer className="mt-auto pt-4">
            <div className="flex items-center gap-2 rounded-2xl border border-primary-200 bg-primary-50/70 p-3 text-sm text-primary-600 backdrop-blur-sm">
              ...
              <span className="text-pretty">
                Workspace preferences save locally. Runtime changes save to
                NyxHive configuration.
              </span>
            </div>
          </footer>
        </div>
...
function ChatDisplaySection() {
  ...
  return (
    <>
      ...
      <LocalModelSuggestionsSection />
      {/* Mobile Navigation removed — not relevant for Nyx Workspace */}
    </>
  )
}

function LocalModelSuggestionsSection() {
  const { settings, updateSettings } = useSettings()
  ...
  return (
    <SettingsSection
      ...
      description="Local chat suggestions for cheaper or stronger models. Runtime routing is configured separately."
      icon={SparklesIcon}
    >
      <SettingsRow
      ...
      <SettingsRow
        ...
        description="Default model for cheaper suggestions."
      >
        <select
          ...
          className="h-9 w-full rounded-lg border border-primary-200 bg-primary-50 px-3 text-sm text-primary-900 outline-none transition-colors focus-visible:ring-2 focus-visible:ring-primary-400 md:max-w-xs"
      ...
      <SettingsRow
        ...
        description="Default model for upgrade suggestions."
      >
        <select
...
const LOADER_STYLES: Array<LoaderStyleOption> = [
  ...
  { value: 'braille-hermes', label: 'Nyx' },
...
type NyxConfigData = {
  ...
  providers: Array<NyxProvider>
  activeProvider: string
  activeModel: string
  nyxHome: string
}

const NYX_API_URL = process.env.NYX_API_URL || 'http://127.0.0.1:8642'
...
function NyxConfigSection({
  ...
  const [data, setData] = useState<NyxConfigData | null>(null)
  ...
  const syncInputsFromData = useCallback((configData: NyxConfigData) => {
  ...
  const fetchConfig = useCallback(async () => {
    const res = await fetch('/api/nyx-config')
    const configData = (await res.json()) as NyxConfigData
    setData(configData)
  ...
  const fetchModelsForProvider = useCallback(async (provider: string) => {
    ...
    try {
      const res = await fetch(
        `/api/nyx-proxy/api/available-models?provider=${encodeURIComponent(provider)}`,
      )
      if (res.ok) {
  ...
  if (!data) {
    return (
      <SettingsSection
        title="Nyx"
        description="Could not load Nyx configuration."
        icon={Settings02Icon}
      >
        <p className="text-sm" style={{ color: 'var(--theme-muted)' }}>
          Make sure NyxHive is running on localhost:3777
        </p>
      </SettingsSection>
    )
  }
  ...
  const renderProvidersAndModels = () => (
    <>
      <SettingsSection
        title="Providers & Models"
        description="Configure provider auth, discovery, and the default runtime model."
        icon={SourceCodeSquareIcon}
      >
        <SettingsRow
          label="Provider"
        ...
        <SettingsRow
          label="Model"
          description="The model Nyx uses for conversations."
        >
          <div className="flex w-full max-w-sm gap-2">
      ...
      <SettingsSection
        title="API Keys"
        description="Manage provider API keys stored in ~/.nyxhive/.env"
        icon={CloudIcon}
      >
        {data.providers
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col gap-3 rounded-xl border border-primary-200 bg-primary-100/40 p-3 md:flex-row md:items-center md:justify-between">
            ...
            <Button
              ...
              variant="outline"
              onClick={() =>
                void navigator.clipboard?.writeText(data.nyxHome)
              }
            >
              Copy config path
            </Button>
  ...
  const renderRuntimeMemory = () => (
    <SettingsSection
      title="Memory"
      description="Configure runtime memory and user profile behavior."
      icon={UserIcon}
    >
      <SettingsRow
  ...
  const renderAdvancedRuntime = () => (
    <>
      <SettingsSection
        title="Backend Status"
        description="Operational backend detail that affects runtime behavior."
        icon={Notification03Icon}
      >
        <SettingsRow
        ...
        <SettingsRow
          label="Active model"
          description="Current default model resolved from runtime config."
        >
          <span className="text-xs font-mono text-primary-700">
            {data.activeModel || 'Not set'}
          </span>
        </SettingsRow>
      </SettingsSection>
      ...
      <SettingsSection
        ...
        description="Where runtime configuration is stored."
        icon={Notification03Icon}
      >
        <SettingsRow
          label="Config location"
          description="Where Nyx stores its configuration."
        >
          <span
            className="text-xs font-mono"
            style={{ color: 'var(--theme-muted)' }}
  ...
  const sectionContent = {
    providers: renderProvidersAndModels(),
    agent: renderAgentBehavior(),
    routing: renderSmartRouting(),
    voice: renderVoice(),
    memory: renderRuntimeMemory(),
    display: renderDisplay(),
    advanced: renderAdvancedRuntime(),
  } as const
