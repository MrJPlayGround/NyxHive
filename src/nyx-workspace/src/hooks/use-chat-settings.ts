...
export type LoaderStyle =
  ...
  | 'lobster'
  | 'logo'
const LEGACY_CHAT_DISPLAY_NAME = 'User'
...
function mergePersistedSettings(
  ...
  if (
    typeof persistedSettings.displayName !== 'string' ||
    persistedSettings.displayName.trim() === '' ||
    persistedSettings.displayName.trim() === LEGACY_CHAT_DISPLAY_NAME
  ) {
    nextSettings.displayName = DEFAULT_CHAT_DISPLAY_NAME
  }
