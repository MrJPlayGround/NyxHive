...
const TAP_DEBUG_STORAGE_KEY = 'nyx:tap-debug'
const LEGACY_TAP_DEBUG_STORAGE_KEY = 'hermes:tap-debug'
const TAP_DEBUG_EVENT = 'nyx:tap-debug-change'
...
function readEnabled(win: TapDebugWindow): boolean {
  if (typeof win.__NYX_TAP_DEBUG__ === 'boolean') {
    return win.__NYX_TAP_DEBUG__
  }
  ...
  try {
    return (
      window.localStorage.getItem(TAP_DEBUG_STORAGE_KEY) === '1' ||
      window.localStorage.getItem(LEGACY_TAP_DEBUG_STORAGE_KEY) === '1'
    )
  } catch {
    return false
  }
