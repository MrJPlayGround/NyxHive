...
export function ProviderWizard({
  ...
  async function handleSaveApiKey() {
    ...
    try {
      ...
      setVerifyState('checking')
      setVerificationMessage(
        `${providerName} API key saved. Nyx is restarting…`,
      )
      setStep('verify')
      ...
      if (!pollingRef.current) {
        ...
        if (found) {
          ...
          setVerificationMessage(
            `Nyx restarted, but ${providerName} models haven't appeared yet. ` +
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-1">
                ...
                <DialogDescription className="text-pretty">
                  ...
                  Nyx config file and are never sent to Studio.
                </DialogDescription>
              </div>
              <Button
                variant="ghost"
              ...
              <section className="mt-5">
                ...
                <div className="mt-3 rounded-xl border border-primary-200 bg-primary-100/70 px-3 py-2">
                  <p className="text-xs text-primary-700 text-pretty">
                    Config file path:{' '}
                    <code className="font-mono">{NYX_CONFIG_PATH}</code>
                  </p>
                </div>
            ...
            {step === 'instructions' && selectedProvider && selectedAuthType ? (
              <section className="mt-5">
                ...
                {selectedAuthType === 'oauth' ? (
                  <>
                    <p className="mt-1 text-sm text-primary-600 text-pretty">
                      OAuth import is not wired into Nyx Workspace yet. Use an
                      API key for this provider until the Nyx auth flow is
                      connected.
                    </p>
                    ...
                    <div className="mt-4 flex flex-col gap-3">
                      <Button
                        ...
                        onClick={function onLaunchOAuth() {
                          ...
                          setVerificationMessage(
                            'No Nyx OAuth command is available for this provider yet. Use API key auth for now.',
                          )
                          setVerifyState('warning')
                          setStep('verify')
                        }}
                      ...
                      <div className="rounded-xl border border-primary-200 bg-primary-100/70 px-3 py-2">
                        ...
                        <p className="mt-1.5 text-xs text-primary-600 text-pretty">
                          Use the API key option until this provider has a Nyx
                          native OAuth bridge.
                        </p>
                      </div>
                      ...
                      <div className="rounded-xl border border-primary-200 bg-primary-100/70 px-3 py-2">
                        <p className="text-xs text-primary-700 text-pretty">
                          Provider reference:{' '}
                          <a
                            href="https://github.com/NousResearch/nyx-agent"
                            target="_blank"
                  ...
                  <>
                    <p className="mt-1 text-sm text-primary-600 text-pretty">
                      ...
                      token import still needs a Nyx-native bridge. Use API key
                      auth until that flow is connected.
                    </p>
                    ...
                    <div className="mt-4 flex flex-col gap-3">
                      <Button
                        ...
                        onClick={function onLaunchCLI() {
                          window.open('/terminal', '_blank')
                          setVerificationMessage(
                            'No Nyx CLI-token import command is available yet. Use API key auth for now.',
                          )
                          setVerifyState('warning')
                          setStep('verify')
                        }}
                    ...
                    <div className="mt-4 flex flex-col gap-3">
                      <div className="flex gap-2">
                        <input
                          ...
                          placeholder={`fake-provider-key or your ${selectedProvider.name} API key`}
                      ...
                      {saveState === 'saved' ? (
                        <p className="text-xs text-green-600">
                          ...
                          />
                          Key saved! Nyx is restarting to apply changes.
                        </p>
                      ) : null}
                    </div>
            ...
            {step === 'verify' ? (
              <section className="mt-5">
                ...
                >
                  ...
                  <p className="mt-1 text-sm text-primary-600 text-pretty">
                    {verificationMessage || 'Waiting for Nyx to respond…'}
                  </p>
                </div>
