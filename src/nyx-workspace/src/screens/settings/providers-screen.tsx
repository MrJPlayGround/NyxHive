...
/**
 * Strip the provider prefix that nyx-agent adds internally via litellm.
...
type NyxConfig = Record<string, unknown>
...
// reverse-proxy deployments where the browser cannot reach Nyx directly.
...
const SETTINGS: Array<SettingDefinition> = [
  ...
  // Thinking/reasoning settings removed — not supported by this backend.
  ...
  for (const providerId of configuredSet) {
    ...
    summaries.push({
      ...
      description:
        ...
        'Configured provider in your local Nyx setup.',
      modelCount,
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ['nyx', 'config'] }),
        queryClient.invalidateQueries({ queryKey: ['nyx-config'] }),
      ])
    ...
    <section className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-4 shadow-sm md:p-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-primary-600">
            ...
            settings saved in{' '}
            <code className="font-mono">~/.nyxhive/config.yaml</code>.
          </p>
        </div>
        <Button
          size="sm"
      ...
      {configQuery.isPending ? (
        ...
        <p className="mt-4 text-sm text-red-500">
          Could not load config - is Nyx running?
        </p>
      ) : (
        <div className="mt-5 space-y-4">
          <ModelConfigSection
            title="Primary Model"
            description="Default provider, model, and base URL used for new Nyx requests."
            value={primaryConfig}
          ...
          <section className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-4 shadow-sm">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm text-primary-600">
                  Optional secondary model Nyx can use if the primary path
                  fails.
                </p>
              </div>
              <Button
          ...
          <section className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-4 shadow-sm">
            ...
            <p className="mt-4 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card2)] px-3 py-2 text-sm text-primary-600">
              ...
              more headroom before Nyx decides a stream has stalled.
            </p>
          </section>
        </div>
      )}
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="mt-1 text-xs text-primary-600">
              API keys stay in your local Nyx config and are never sent to
              Studio.
            </p>
          </div>
        ...
        {modelsQuery.error ? (
          <div className="rounded-xl border border-primary-200 bg-white px-4 py-3">
            <p className="mb-2 text-sm text-primary-700">
              Unable to load providers right now. Check your Nyx connection.
            </p>
            <Button
              variant="outline"
              size="sm"
...
export function ProvidersScreen({ embedded = false }: ProvidersScreenProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async (_, variables) => {
      await queryClient.invalidateQueries({ queryKey: ['nyx', 'config'] })
        ...
        <section className="mx-auto w-full max-w-[1480px] space-y-5">
          <header className="flex flex-col gap-4 rounded-xl border border-primary-200 bg-primary-50/80 px-5 py-4 shadow-sm">
            <div className="space-y-1">
              ...
              <p className="text-sm text-primary-600">
                Configure providers plus Nyx defaults in one place.
              </p>
            </div>
