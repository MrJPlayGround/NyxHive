...
import type { NyxSession } from '@/server/nyx-api'
...
import { deriveSetupReadiness } from '@/screens/chat/workspace-ux-state'
...
function OperationalBrief({
  ...
  readiness: ReturnType<typeof deriveSetupReadiness>
  onPrimaryAction: () => void
}) {
  const statusColor =
    readiness.status === 'ready'
      ? '#22c55e'
      : readiness.status === 'needs-config'
        ? '#f59e0b'
        : '#ef4444'
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="shrink-0 rounded-lg border border-[var(--theme-border)] bg-[var(--theme-card2)] px-3 py-2 text-xs font-semibold text-ink transition-colors hover:border-[var(--theme-accent-border)]"
      >
...
function ActivityChart({ sessions }: { sessions: Array<NyxSession> }) {
...
export function DashboardScreen() {
  ...
  const sessionsQuery = useQuery({
    ...
    // listSessions() — the latter calls the gateway via NYX_API_URL which is
    ...
    queryFn: async () => {
      ...
      return (data.sessions ?? []).map((s) => ({
        ...
        title: (s.label ?? s.title ?? s.derivedTitle ?? s.key ?? s.id) as string,
        model: (s.model ?? s.agent ?? '') as string,
  ...
  const sessions = (sessionsQuery.data ?? []) as NyxSession[]
  ...
  return (
    <div className="min-h-full">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-2 h-12" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
        ...
        <button
          ...
          onClick={() => {
            ...
            const cur = document.documentElement.getAttribute('data-theme') || 'nyx-official'
            const nextDataTheme = LIGHT_DARK_PAIRS[cur] || (isDark ? 'nyx-official-light' : 'nyx-official')
      ...
      {/* ── Header: Agent logo + Quick Actions ── */}
      ...
      <OperationalBrief
        readiness={readiness}
        onPrimaryAction={() => {
          if (readiness.status === 'blocked' || readiness.status === 'needs-config') {
            void navigate({ to: '/settings' })
            return
          }
          if (stats.totalSessions > 0 && recentSessions[0]?.id) {
            void navigate({
              to: '/chat/$sessionKey',
              params: { sessionKey: recentSessions[0].id },
            })
            return
          }
          void navigate({
