...
/**
 * Strip Nyx system metadata from user messages.
...
export function streamingTextForMessageList(
  isStreaming: boolean,
  streamingText: string | undefined,
): string | undefined {
  if (isStreaming) return ''
  return streamingText && streamingText.length > 0 ? streamingText : undefined
}
...
export function normalizeSessions(
  ...
  return rows.map((session) => {
    const canonicalId =
      normalizeStringId(session.key) ||
      normalizeStringId(session.id) ||
      normalizeStringId(session.friendlyId)
    const key = canonicalId || deriveFriendlyIdFromKey(session.friendlyId)
    const friendlyIdCandidate =
      normalizeStringId(session.friendlyId) || deriveFriendlyIdFromKey(key)
...
export const missingAuthMessage =
  `${WORKSPACE_AGENT_NAME} connection failed. Make sure NyxHive is running and NYX_WORKSPACE_API_URL is set correctly.`
