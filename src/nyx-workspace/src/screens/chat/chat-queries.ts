...
export function removeHistoryMessageByClientId(
  ...
  const remove = (messages: Array<ChatMessage>) =>
  ...
  updateHistoryMessages(queryClient, friendlyId, sessionKey, remove)
  ...
  for (const [queryKey, data] of historyQueries) {
    if (
      ...
      JSON.stringify(chatQueryKeys.history(friendlyId, sessionKey))
    ) {
      continue
    }
    const messages = Array.isArray(data?.messages) ? data.messages : []
    const nextMessages = remove(messages)
  ...
  queryClient.setQueryData(
    ...
    function update(messages: unknown) {
      ...
      return (messages as Array<SessionMeta>).filter((session) => {
        return !ids.has(session.key) && !ids.has(session.friendlyId)
      })
    },
  )

  for (const [queryKey] of queryClient.getQueriesData<HistoryResponse>({
    queryKey: ['chat', 'history'],
  })) {
    if (!Array.isArray(queryKey)) continue
    const matchesDeletedSession = queryKey.some((part) => {
      return typeof part === 'string' && ids.has(part)
    })
    if (!matchesDeletedSession) continue
