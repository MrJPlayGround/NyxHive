...
function ChatHeaderComponent({
  ...
  const modelLabel = _agentModel
    ? _agentModel.split('/').pop()?.replace(/-/g, ' ') || _agentModel
    : 'model'
  const statusLabel =
    statusMode === 'tool' && activeToolName
      ? activeToolName
  ...
  const handleOpenAgentDetails = useCallback(() => {
    ...
    window.dispatchEvent(new CustomEvent('nyx:chat-agent-details'))
  }, [onOpenAgentDetails])
      ...
      >
        <div className="px-3 h-12 flex items-center gap-0">
          ...
          <button
            ...
            className="flex min-w-0 max-w-[60vw] flex-col items-start rounded-full bg-primary-100/70 px-3 py-1.5 hover:bg-primary-200/80 dark:bg-neutral-700/80 dark:hover:bg-neutral-600/80 transition-colors"
          ...
          >
            ...
            <span className="mt-0.5 flex max-w-full items-center gap-1 text-[10px] text-primary-500">
              <span
                className={cn(
                  'size-1.5 rounded-full',
                  agentConnected ? 'bg-emerald-500' : 'bg-red-500',
                )}
              />
  ...
  return (
    <div ref={wrapperRef} className="shrink-0 bg-surface">
      <div className="px-4 h-12 flex items-center">
        ...
        <div className="mr-2 hidden min-w-0 max-w-[22rem] items-center gap-2 rounded-full border border-primary-200 bg-primary-50/70 px-2.5 py-1 text-[11px] text-primary-600 lg:flex">
          <span
            className={cn(
