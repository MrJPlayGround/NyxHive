...
function classifyConnectionError(
  ...
  if (!normalizedError && !status) {
    return {
      title: 'Not connected',
      description: `${WORKSPACE_DISPLAY_NAME} can't reach ${WORKSPACE_AGENT_NAME}.`,
      action: `Check that ${WORKSPACE_AGENT_NAME} is running, then try again.`,
    }
  }
  ...
  if (
    ...
    return {
      ...
      description: `${WORKSPACE_AGENT_NAME} rejected the connection token.`,
      action: `Go to Settings -> Advanced -> ${WORKSPACE_AGENT_NAME} to update your token.`,
    }
  }
  ...
  if (
    ...
    return {
      ...
      description: `This device isn't paired with ${WORKSPACE_AGENT_NAME} yet.`,
      action: `Check ${WORKSPACE_AGENT_NAME} connection.`,
    }
  }

  if (lower.includes('econnrefused') && lower.includes('8642')) {
    return {
      title: 'NyxHive gateway not running',
      description: 'The NyxHive gateway is not running.',
      action: 'Run: bun run hive:start',
    }
  }
  ...
  if (
    ...
    return {
      title: `${WORKSPACE_AGENT_NAME} unreachable`,
      description: `Can't connect to ${WORKSPACE_AGENT_NAME} at the configured URL.`,
      action: `Make sure ${WORKSPACE_AGENT_NAME} is running and the URL is correct.`,
    }
  }
  ...
  return {
    ...
    action: `Try refreshing or check Settings -> Advanced -> ${WORKSPACE_AGENT_NAME}.`,
  }
}
...
export function ConnectionStatusMessage({
  ...
  useEffect(() => {
    ...
    window.addEventListener('nyx:health-restored', handleRestored)
    return () =>
      window.removeEventListener('nyx:health-restored', handleRestored)
  }, [])
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 text-xs">
          <p className="font-medium">
            {isChecking
              ? `Connecting to ${WORKSPACE_AGENT_NAME}...`
              : errorInfo.title}
          </p>
          {!isChecking ? (
            <>
