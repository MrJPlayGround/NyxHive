...
import { CHAT_SURFACE_MAX_WIDTH } from '../message-layout'
import { WORKSPACE_AGENT_NAME } from '@/lib/workspace-branding'
import type { ComposerState } from './chat-composer-state'
import {
  deriveComposerState,
  shouldAcceptSlashSuggestionKey,
} from './chat-composer-state'
import type { ConversationModePreference } from '../conversation-mode-router'
...
type ConversationMode = Exclude<ConversationModePreference, 'auto'>
...
type ChatComposerProps = {
  ...
  onConversationModeChange?: (mode: ConversationModePreference) => void
  onAbort?: () => void
...
function reasoningLabel(level: ThinkingLevel): string {
  if (level === 'off') return 'Reasoning off'
  if (level === 'medium') return 'Reasoning medium'
  return `Reasoning ${level}`
}

function conversationModeLabel(mode: ConversationModePreference): string {
  if (mode === 'auto') return 'Auto Mode'
  if (mode === 'quick') return 'Quick Chat'
  if (mode === 'task') return 'Task Mode'
  if (mode === 'deep') return 'Deep Mode'
  return 'Build Mode'
}

/** Returns true if the model should auto-default to high reasoning in the workspace UI. */
function isHighReasoningDefaultModel(model: string): boolean {
...
function isNyxCatalogEntry(
  entry: NyxCatalogEntry | null,
): entry is NyxCatalogEntry {
  return entry !== null
}

type NyxProviderOption = {
  id: string
  label: string
  authenticated: boolean
}

type NyxAvailableModelsResponse = {
  provider: string
  models: Array<{ id: string; description: string }>
  providers: Array<NyxProviderOption>
}
  ...
  const models = rawModels
    .map((entry) => {
      ...
      const provider =
        ...
        (id.includes('/') ? id.split('/')[0] : 'nyx-agent')
...
async function fetchModels(): Promise<{
  ...
  const nativeModels = await fetchApiModels().catch(() => null)
  if (nativeModels?.models.length) return nativeModels

  // Prefer Nyx's current provider models; fetch other providers lazily if needed.
  try {
    const richRes = await fetch('/api/nyx-proxy/api/available-models')
    if (richRes.ok) {
      const richData = (await richRes.json()) as NyxAvailableModelsResponse
      ...
      if (models.length === 0) {
        try {
          const fallbackRes = await fetch('/api/nyx-proxy/v1/models')
          if (fallbackRes.ok) {
      ...
      if (currentProvider && models.length === 0) {
        ...
        try {
          const cfgRes = await fetch('/api/nyx-proxy/api/config')
          if (cfgRes.ok) {
        ...
        const otherResults = await Promise.allSettled(
          otherVisible.map(async (provider) => {
            const res = await fetch(
              `/api/nyx-proxy/api/available-models?provider=${encodeURIComponent(provider.id)}`,
            )
            if (!res.ok) return []
            const data = (await res.json()) as NyxAvailableModelsResponse
...
async function fetchModelsForProvider(
  ...
  const response = await fetch(
    `/api/nyx-proxy/api/available-models?provider=${encodeURIComponent(normalizedProvider)}`,
  )
  if (!response.ok) {
    throw new Error(`Nyx models request failed (${response.status})`)
  }

  const payload = (await response.json()) as NyxAvailableModelsResponse
...
async function switchModel(
  ...
  // Write the model change through the workspace config API.
  const patch: Record<string, string> = { model: modelId }
  if (modelProvider) patch.provider = modelProvider

  const response = await fetch('/api/nyx-proxy/api/config', {
    method: 'PATCH',
  ...
  return {
    ok: true,
    resolved: {
      modelProvider: modelProvider || 'nyx-agent',
      model: modelId,
    },
  }
}
...
function toDraftStorageKey(sessionKey?: string): string {
  return `nyx-draft-${normalizeDraftSessionKey(sessionKey)}`
}

function toLegacyDraftStorageKey(sessionKey?: string): string {
...
function getComposerStateTone(state: ComposerState): string {
  if (state.intent === 'queued-followup') {
    return 'border-amber-300/70 bg-amber-50/80 text-amber-950 dark:bg-amber-950/25 dark:text-amber-100'
  }
  if (state.intent === 'running') {
    return 'border-accent-400/40 bg-accent-50/65 text-accent-900 dark:bg-accent-950/20 dark:text-accent-100'
  }
  if (state.intent === 'attaching') {
    return 'border-orange-300/60 bg-orange-50/80 text-orange-950 dark:bg-orange-950/20 dark:text-orange-100'
  }
  if (state.intent === 'code' || state.intent === 'command') {
    return 'border-primary-300/70 bg-primary-50/85 text-primary-900 dark:bg-primary-950/20 dark:text-primary-100'
  }
  return 'border-primary-200/70 bg-primary-50/55 text-primary-800 dark:bg-primary-950/10 dark:text-primary-100'
}

function getComposerDotTone(state: ComposerState): string {
  if (state.intent === 'queued-followup') return 'bg-amber-500 animate-pulse'
  if (state.intent === 'running') return 'bg-accent-500 animate-pulse'
  if (state.intent === 'attaching') return 'bg-orange-500 animate-pulse'
  if (state.intent === 'code' || state.intent === 'command')
    return 'bg-primary-500'
  return 'bg-primary-400'
}

function getSendActionLabel(state: ComposerState): string {
  if (state.intent === 'queued-followup') return 'Steer active run'
  if (state.intent === 'code' || state.intent === 'attaching') {
    return 'Run with context'
  }
  if (state.intent === 'command') return 'Run action'
  return 'Send to Nyx'
}

function ComposerStateStrip({
  state,
  compact = false,
}: {
  state: ComposerState
  ...
  return (
    <div
      className={cn(
        'mx-3 flex items-center justify-between gap-3 rounded-2xl border px-3 py-2 transition-colors duration-200',
        compact ? 'mt-2' : '',
        getComposerStateTone(state),
      )}
    >
      <div className="flex min-w-0 items-start gap-2">
        ...
        <div className="min-w-0">
          ...
          <div
            className={cn(
              'truncate text-[11px] leading-4 opacity-75',
      ...
      {state.badges.length > 0 ? (
        <div className="hidden max-w-[46%] shrink-0 items-center justify-end gap-1 md:flex">
          {state.badges.slice(0, 3).map((badge) => (
            <span
              key={badge}
              className="max-w-[8.5rem] truncate rounded-full border border-current/10 bg-white/45 px-2 py-0.5 text-[10px] font-medium leading-4 opacity-80 dark:bg-white/10"
            >
              {badge}
            </span>
          ))}
...
function ChatComposerComponent({
  ...
  conversationMode: externalConversationMode,
  ...
  const [internalConversationMode, setInternalConversationMode] =
    useState<ConversationModePreference>('build')
  const conversationMode = externalConversationMode ?? internalConversationMode
  // Thinking toggle is currently owned by chat-screen; keep local state for type compat.
  ...
  const handleConversationModeSelect = useCallback(
  ...
  // Removed: auto-switching the model. The workspace respects the
  // model/provider configured for Nyx. Users switch
  // via the model selector or Settings page.

  // When model switches to a high-reasoning default and thinking is 'off', auto-upgrade to high reasoning
  const prevModelRef = useRef('')
  useEffect(() => {
    if (!currentModel || currentModel === prevModelRef.current) return
    prevModelRef.current = currentModel
    if (isHighReasoningDefaultModel(currentModel) && thinkingLevel === 'off') {
      if (onThinkingLevelChange) {
        onThinkingLevelChange('high')
      } else {
        setInternalThinkingLevel('high')
      }
    }
  }, [currentModel, thinkingLevel, onThinkingLevelChange])

  const isModelSwitcherDisabled = disabled || modelSwitchMutation.isPending
  const draftStorageKey = useMemo(() => toDraftStorageKey(sessionKey), [
    sessionKey,
  ])
  const legacyDraftStorageKey = useMemo(
    () => toLegacyDraftStorageKey(sessionKey),
    [sessionKey],
  )
  ...
  const modelButtonLabel =
    currentSelectedModel || currentModel || configuredModel || WORKSPACE_AGENT_NAME
  ...
  const sendActionLabel = getSendActionLabel(composerState)
  const promptPlaceholder = isMobileViewport
    ? 'Message Nyx...'
    : 'Give Nyx a task, context, or question...'
  ...
  const handlePromptKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      if (isSlashMenuOpen) {
        ...
        if (shouldAcceptSlashSuggestionKey(event.key, event.shiftKey)) {
          event.preventDefault()
  ...
  const composerWrapperStyle = useMemo(() => {
    if (!isMobileViewport)
      return { maxWidth: CHAT_SURFACE_MAX_WIDTH } as CSSProperties
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ComposerStateStrip state={composerState} compact={isMobileViewport} />
          ...
          <>
            <div className="flex px-3 pt-2">
              <button
                ...
                className="inline-flex h-7 max-w-full items-center gap-1.5 rounded-full border border-neutral-200 bg-neutral-50 px-2.5 text-[11px] font-medium text-neutral-600 shadow-sm transition-colors active:bg-neutral-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-neutral-800 dark:bg-white/5 dark:text-neutral-300"
                aria-label={`Current model ${modelButtonLabel}`}
                          ...
                          if (allModels.length === 0) {
                            return (
                              <div className="p-4 text-center text-sm text-neutral-500">
                                ...
                                <p className="text-xs">
                                  Check your Nyx provider configuration.
                                </p>
                              </div>
                            )
                          }
          ...
          <>
            ...
            <PromptInputActions className="justify-between px-1.5 md:px-3 gap-0.5 md:gap-2">
              <div className="flex min-w-0 flex-1 items-center gap-0 md:gap-1">
                ...
                <div
                  className="ml-1 hidden shrink-0 items-center rounded-full border border-neutral-200 bg-neutral-50 p-0.5 dark:border-neutral-800 dark:bg-neutral-900 sm:flex"
                  aria-label="Conversation mode"
                  title={conversationModeLabel(conversationMode)}
                >
                  {(['auto', 'quick', 'task', 'build', 'deep'] as const).map(
                    (mode) => (
                      <button
                        key={mode}
                        type="button"
                        onClick={() => handleConversationModeSelect(mode)}
                        disabled={disabled}
                        className={cn(
                          'h-6 min-w-8 rounded-full px-2 text-[10px] font-medium uppercase transition-colors',
                        ...
                        aria-label={`Set conversation mode ${mode}`}
                      >
                        {mode}
                      </button>
                    ),
                ...
                <div
                  ...
                  title={reasoningLabel(thinkingLevel)}
                >
              ...
              <div className="ml-1 flex shrink-0 items-center gap-0.5 md:gap-1">
                ...
                {isLoading ? (
                  ...
                  <>
                    <PromptInputAction tooltip={sendActionLabel}>
                      <Button
                        type="button"
                        onClick={handleSubmit}
                        disabled={submitDisabled}
