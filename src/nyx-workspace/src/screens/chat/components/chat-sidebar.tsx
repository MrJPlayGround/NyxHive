...
import { useInspectorStore } from '@/components/inspector/inspector-panel'
import {
...
function ThemeToggleMini() {
  ...
  const currentDataTheme = typeof document !== 'undefined'
    ? document.documentElement.getAttribute('data-theme') || 'nyx-official'
    : 'nyx-official'
  ...
  return (
    <button
      type="button"
      onClick={() => {
        const nextDataTheme = LIGHT_DARK_PAIRS[currentDataTheme] || (isDark ? 'nyx-official-light' : 'nyx-official')
        setTheme(nextDataTheme as ThemeId)
...
const LAST_ROUTE_KEY = 'nyx-sidebar-last-route'
const LEGACY_LAST_ROUTE_KEY = 'hermes-sidebar-last-route'

function setLastRoute(section: string, route: string) {
  try {
    const stored =
      localStorage.getItem(LAST_ROUTE_KEY) ||
      localStorage.getItem(LEGACY_LAST_ROUTE_KEY)
    const map = stored ? (JSON.parse(stored) as Record<string, string>) : {}
    map[section] = route
    localStorage.setItem(LAST_ROUTE_KEY, JSON.stringify(map))
    localStorage.removeItem(LEGACY_LAST_ROUTE_KEY)
  } catch {
    // ignore
  }
}
...
function ChatSidebarComponent({
  ...
  useEffect(() => {
    function handleOpenSettingsEvent(event: Event) {
      ...
      handleOpenSettings(detail?.section === 'appearance' ? 'appearance' : 'nyx')
    }
  ...
  const knowledgeRoutes = ['/memory', '/skills', '/profiles']
  const systemRoutes = ['/settings', '/logs']
  const [lastRouteMap, setLastRouteMap] = useState<Record<string, string>>({})
  ...
  useEffect(() => {
    try {
      const stored =
        ...
        localStorage.getItem(LEGACY_LAST_ROUTE_KEY)
      setLastRouteMap(
        stored ? (JSON.parse(stored) as Record<string, string>) : {},
      )
    } catch {
      setLastRouteMap({})
    }
  ...
  useEffect(() => {
    const nextSection = mainRoutes.includes(pathname)
      ? 'main'
      : knowledgeRoutes.includes(pathname)
        ? 'knowledge'
        : systemRoutes.includes(pathname)
          ? 'system'
          : null
    if (!nextSection) return
    setLastRoute(nextSection, pathname)
    setLastRouteMap((current) => ({ ...current, [nextSection]: pathname }))
  }, [pathname])

  const mainNav = mainRoutes.includes(pathname)
    ? pathname
    : lastRouteMap.main || '/chat'
  const knowledgeNav = knowledgeRoutes.includes(pathname)
    ? pathname
    : lastRouteMap.knowledge || '/memory'
  const _systemNav = systemRoutes.includes(pathname)
    ? pathname
    : lastRouteMap.system || '/settings'

  const transition = {
    duration: 0.15,
    ease: isCollapsed ? 'easeIn' : 'easeOut',
  } as const
  const workspaceRuntimeStatus = useMemo(() => {
    if (typeof window === 'undefined') return formatWorkspaceRuntimeStatus()
    return formatWorkspaceRuntimeStatus({ appPort: window.location.port })
  }, [])
  ...
  const [bulkDeleteSessions, setBulkDeleteSessions] = useState<Array<SessionMeta>>([])
  ...
  function handleOpenBulkDelete(targetSessions: Array<SessionMeta>) {
    const deletable = targetSessions.filter(
      (session) => session.key || session.friendlyId,
    )
    if (deletable.length === 0) return
    setBulkDeleteSessions(deletable)
  ...
  function handleConfirmDelete() {
    if (bulkDeleteSessions.length > 0) {
      const activeWillBeDeleted = bulkDeleteSessions.some(
        (session) => session.friendlyId === activeFriendlyId,
      )
      if (activeWillBeDeleted && onActiveSessionDelete) {
        onActiveSessionDelete()
      }
      const targets = [...bulkDeleteSessions]
      clearDeleteState()
      void (async () => {
  ...
  return (
    ...
    >
          ...
          {!isVisuallyCollapsed ? (
            ...
            >
              ...
              >
                ...
                />
                <span className="text-sm font-semibold tracking-tight" style={{ color: 'var(--theme-text)' }}>{WORKSPACE_DISPLAY_NAME}</span>
              </Link>
            </motion.div>
          ) : null}
        </AnimatePresence>
              ...
              >
                <div className="flex-1 min-h-0">
                  <SidebarSessions
                    ...
                    activeRuntimeSessionKeys={activeRuntimeSessionKeys}
                    onSelect={onSelectSession}
                    onRename={handleOpenRename}
                    onDelete={handleOpenDelete}
                    onDeleteMany={handleOpenBulkDelete}
      ...
      <div className="px-2 py-2.5 border-t shrink-0 theme-border theme-panel">
        {!isVisuallyCollapsed ? (
          <div
            data-tour="workspace-runtime-status"
            className="mb-2 rounded-lg border border-primary-200/70 bg-primary-50/50 px-2.5 py-1.5 text-[10px] font-medium leading-4 text-primary-500 dark:border-neutral-800 dark:bg-neutral-900/60 dark:text-neutral-400"
        ...
        <div className={cn(
          ...
          {!isVisuallyCollapsed && (
            <div className="flex items-center gap-0.5">
              <button
                type="button"
                onClick={() => handleOpenSettings('nyx')}
      ...
      <SessionDeleteDialog
        ...
        sessionCount={bulkDeleteSessions.length || 1}
...
function areSidebarPropsEqual(
  ...
  if (prevProps.activeRuntimeSessionKeys !== nextProps.activeRuntimeSessionKeys)
    return false
