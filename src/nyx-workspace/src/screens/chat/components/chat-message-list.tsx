 ...
 *  clears, giving the final assistant message time to hydrate from the
 *  send-stream/history handoff.  The grace exits early as soon as streaming
 *  text or a new assistant message appears, so this is a ceiling, not a delay. */
const THINKING_GRACE_PERIOD_MS = 2_400

type ActiveToolCall = Omit<StreamToolCall, 'phase'> & { phase: string }

/** Map tool names to human-readable status strings */
const TOOL_STATUS_MAP: Record<string, string> = {
  memory_search: `${WORKSPACE_AGENT_NAME} is checking context...`,
  memory_get: `${WORKSPACE_AGENT_NAME} is checking context...`,
  web_search: `${WORKSPACE_AGENT_NAME} is checking sources...`,
  web_fetch: `${WORKSPACE_AGENT_NAME} is checking sources...`,
  cron: 'Managing schedules…',
  message: 'Sending message…',
  gateway: 'Managing gateway…',
  canvas: 'Rendering canvas…',
  voice_call: 'Making call…',
  pdf: `${WORKSPACE_AGENT_NAME} is checking context...`,
  todo: 'Managing tasks…',
  Read: `${WORKSPACE_AGENT_NAME} is checking context...`,
  read: `${WORKSPACE_AGENT_NAME} is checking context...`,
  Write: `${WORKSPACE_AGENT_NAME} is working...`,
  write: `${WORKSPACE_AGENT_NAME} is working...`,
  Edit: `${WORKSPACE_AGENT_NAME} is working...`,
  edit: `${WORKSPACE_AGENT_NAME} is working...`,
  exec: `${WORKSPACE_AGENT_NAME} is working...`,
...
function getToolStatusLabel(toolName: string): string {
  return TOOL_STATUS_MAP[toolName] ?? `${WORKSPACE_AGENT_NAME} is working...`
}
...
function getStableMessageTimestampKey(message: ChatMessage): string {
  const raw = message as Record<string, unknown>
  for (const key of ['createdAt', 'created_at', 'timestamp', 'time', 'ts']) {
    const value = raw[key]
    if (typeof value === 'number' && Number.isFinite(value)) {
      return String(value < 1_000_000_000_000 ? value * 1000 : value)
    }
...
function getMessageAttachmentDedupeKey(message: ChatMessage): string {
...
function formatVisibleThinkingLabel(thinking?: string): string | null {
            ...
            className={cn(
              ...
              step.state === 'error'
                ...
                : step.state === 'done'
                  ...
                  : step.state === 'active'
                    ? 'bg-primary-500'
                    : 'bg-primary-300/70 dark:bg-primary-700',
            )}
          />
          <span className="min-w-0 flex-1">
            <span className="block truncate text-primary-600 dark:text-primary-300">
              {step.label}
            </span>
            {step.detail ? (
              <span className="block truncate text-[10px] text-primary-400 dark:text-primary-500">
                {step.detail}
              </span>
            ) : null}
          </span>
...
function ThinkingBubble({
  ...
  const allTools = useMemo(
    () =>
      liveToolActivity.length > 0
        ? [...liveToolActivity].reverse().map((t, index) => ({
            id: `${t.name}-${t.timestamp}-${index}`,
            name: t.name,
  ...
  const activityToolSections = useMemo<Array<InlineToolSection>>(
    () =>
      allTools.map((tool, index) => {
        ...
        return {
          ...
          type: tool.name,
          input: coerceToolInput(tool.args),
          preview: tool.preview,
          outputText: tool.result ?? '',
          errorText: isError ? tool.result || 'Tool failed' : undefined,
          state: isError
            ? 'output-error'
            : isDone
              ? 'output-available'
  ...
  const activitySteps = useMemo(
    () =>
      buildStreamingActivityTimeline({
        ...
        lifecycleEvents,
        toolSections: activityToolSections,
        formatToolLabel: defaultToolActivityLabel,
      }),
    [activityToolSections, lifecycleEvents, streamingThinking],
  )
  ...
  const visibleThinkingLabel = formatVisibleThinkingLabel(streamingThinking)
  const statusLabel = isCompacting
    ? `${WORKSPACE_AGENT_NAME} is checking context...`
    : activeToolName
      ? getToolStatusLabel(activeToolName)
      : visibleThinkingLabel || `${WORKSPACE_AGENT_NAME} is thinking...`
  ...
  const completedResearchSteps = researchCard
    ...
    : 0
  const canExpandDetails = activitySteps.length > 0 || canExpandResearch
        ...
        <div className="relative flex flex-col gap-2 px-4 py-3">
          <div className="flex items-start justify-between gap-3">
            ...
            {canExpandDetails ? (
              <button
                ...
                aria-label={detailsOpen ? 'Collapse run details' : 'Expand run details'}
                title={detailsOpen ? 'Collapse run details' : 'Expand run details'}
              >
                <HugeiconsIcon
                  icon={detailsOpen ? ArrowUp01Icon : ArrowExpand01Icon}
                  size={14}
                  strokeWidth={1.8}
                />
              </button>
          ...
          {isStale ? (
            <span className="text-[11px] text-amber-500 dark:text-amber-400 animate-pulse">
              {isVeryStale
                ? `${WORKSPACE_AGENT_NAME} is working...`
                : `${WORKSPACE_AGENT_NAME} is working...`}
            </span>
          ) : null}

          {detailsOpen && activitySteps.length > 0 ? (
            <div className="border-t border-primary-200/60 pt-2 text-[11px] leading-5 text-primary-500 dark:border-primary-800/60 dark:text-primary-400">
              <ActivityTimelineRows steps={activitySteps} />
            </div>
          ) : null}
        </div>

        {detailsOpen && expandedResearchCard && !expandedResearchCard.collapsed ? (
...
function ChatMessageListComponent({
  ...
  const displayMessages = useMemo(() => {
    ...
    const seenStableContent = new Set<string>()
    const deduped = filteredMessages.filter((message) => {
      ...
      const stableTimestamp = getStableMessageTimestampKey(message)
      const text = textFromMessage(message).trim().replace(/\s+/g, ' ')
      const attachmentKey = getMessageAttachmentDedupeKey(message)
      if (
        stableTimestamp &&
        (text || attachmentKey) &&
        ...
        if (seenStableContent.has(contentKey)) return false
        seenStableContent.add(contentKey)
      }
  ...
  useEffect(() => {
    ...
    if (!wasWaiting && waitingForResponse && thinkingGrace) {
      if (thinkingGraceTimerRef.current) {
        clearTimeout(thinkingGraceTimerRef.current)
        thinkingGraceTimerRef.current = null
      }
  ...
  }, [displayEntries, thinkingGrace, waitingForResponse]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const showTypingIndicator = (() => {
    ...
    const hasInThreadStreamingActivity =
      isStreaming && (activeToolCalls.length > 0 || lifecycleEvents.length > 0)
    ...
    if (isStreaming && !streamingText && streamingThinking?.trim()) return false
  ...
  const normalizedStreamingToolCalls = useMemo<Array<StreamToolCall>>(
    () =>
