...
import {
  ...
  normalizeStreamToolPhase,
  selectStreamingStatusToolSections,
  selectVisibleToolSections,
  shouldShowStreamingStatusWindow,
  type StreamingActivityState,
  type InlineToolSection,
  type LifecycleEvent,
  type StreamToolCall,
  type StreamingActivityStep,
} from './stream-activity'
import { CHAT_MESSAGE_BUBBLE_WIDTH_CLASS } from '../message-layout'
import {
  deriveActionRisk,
...
function riskBadgeClass(tier: ActionRiskTier): string {
  if (tier === 'high') {
...
function ArtifactSummaryBar({ summary }: { summary: ArtifactSummary }) {
  return (
    <div className="ml-9 mt-1 flex w-fit max-w-[min(100%,42rem)] items-center gap-2 rounded-lg border border-primary-200 bg-primary-50/80 px-3 py-1.5 text-[11px] text-primary-600">
      <span className="font-semibold text-primary-800">{summary.label}</span>
...
function InlineToolSectionItem({
  ...
  const actionRisk = deriveActionRisk(toolSection.type, toolSection.input)
        ...
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <span
            className={cn(
              ...
              riskBadgeClass(actionRisk.tier),
            )}
            title={actionRisk.detail}
          >
            {actionRisk.label}
          </span>
...
function StreamingActivityRows({
  steps,
  dense = false,
}: {
  ...
  return (
    <ol className={cn(dense ? 'space-y-1' : 'space-y-1.5')}>
      {steps.map((step) => (
        <li
          key={step.key}
          className={cn(
            'flex items-start gap-2 rounded-md transition-colors',
            step.state === 'active' && !dense && 'bg-primary-100/60 px-2 py-1',
          )}
        >
          <span
            className={cn(
          ...
          <span className="min-w-0 flex-1">
            ...
            {step.detail ? (
              <span className="block truncate text-[10px] text-primary-400">
                {step.detail}
              </span>
            ) : null}
          </span>
...
function StreamingActiveStep({
  ...
  return (
    <div className="mt-2 rounded-lg border border-primary-200/80 bg-primary-50 px-3 py-2 shadow-inner">
      <div className="flex items-start gap-2">
        ...
        <span className="min-w-0 flex-1">
          ...
          {step.detail ? (
            <span className="block truncate pt-0.5 text-[10px] text-primary-400">
              {step.detail}
            </span>
          ) : null}
        </span>
...
function StreamingStatusWindow({
  activityState,
  isStreaming,
}: {
  activityState: StreamingActivityState
  isStreaming: boolean
}) {
  const { header, sublabel, elapsedLabel, activeStep, recentSteps, allSteps } =
    activityState

  return (
    <div className="w-full max-w-[min(100%,720px)]">
      <Collapsible defaultOpen={false}>
        <div className="rounded-lg border border-primary-200 bg-primary-50/80 px-3 py-2 text-primary-700 shadow-sm transition-colors hover:border-primary-300 hover:bg-primary-50">
          <CollapsibleTrigger className="w-full justify-start gap-2">
            <span className="flex size-6 shrink-0 items-center justify-center rounded-md bg-accent-500/10 text-accent-600">
            ...
            <span className="min-w-0 flex-1 text-left">
              ...
              <span className="block truncate text-[10px] font-medium text-primary-400">
                {sublabel}
              </span>
            </span>
            {elapsedLabel ? (
              <span className="shrink-0 font-mono text-[10px] tabular-nums text-primary-400">
                {elapsedLabel}
              </span>
            ) : null}
            ...
            <HugeiconsIcon
              ...
              size={16}
              strokeWidth={1.5}
              className="shrink-0 opacity-60 transition-transform duration-150 group-data-panel-open:rotate-180"
            />
          </CollapsibleTrigger>

          <StreamingActiveStep step={activeStep} />

          {recentSteps.length > 0 ? (
            <div className="mt-2 border-t border-primary-200/60 pt-2 text-[11px] leading-5">
              <StreamingActivityRows steps={recentSteps} dense />
            </div>
          ) : null}
        </div>
        <CollapsiblePanel className="mt-1">
          <div className="border-t border-primary-200/70 px-3 py-2 text-[11px] leading-5 text-primary-500">
            {allSteps.length > 0 ? (
              <div className="space-y-2">
                <div className="text-[9px] font-semibold uppercase tracking-widest text-primary-400">
                  Trace detail
                </div>
                <StreamingActivityRows steps={allSteps} />
              </div>
            ) : (
...
function MessageItemComponent({
  ...
  useEffect(() => {
    const hasStreamingStatusActivity =
      Boolean(thinking) || hasLifecycleEvents || hasStreamToolCalls
    if (
      !effectiveIsStreaming ||
      hasRevealedText ||
      !hasStreamingStatusActivity
    ) {
  ...
  const visibleToolSections = useMemo(
    () => selectVisibleToolSections(finalToolSections, effectiveIsStreaming),
    [finalToolSections, effectiveIsStreaming],
  )
  const streamingStatusToolSections = useMemo(
    () =>
      effectiveIsStreaming
        ? selectStreamingStatusToolSections(inlineToolSections)
        : [],
    [effectiveIsStreaming, inlineToolSections],
  )
  const streamingActivityState = useMemo(
    () =>
      deriveStreamingActivityState({
        thinking,
        lifecycleEvents: effectiveLifecycleEvents,
        toolSections: streamingStatusToolSections,
        formatToolLabel: (toolSection) =>
          formatToolDisplayLabel(toolSection.type, toolSection.input),
    ...
    [
      effectiveIsStreaming,
      effectiveLifecycleEvents,
      hasRevealedText,
      streamingStatusToolSections,
      thinking,
      thinkingElapsedSeconds,
    ],
  )
  const showStreamingStatus = shouldShowStreamingStatusWindow({
    isUser,
    isStreaming: effectiveIsStreaming,
    hasRevealedText,
    hasStatusActivity:
      Boolean(thinking) ||
      hasLifecycleEvents ||
      streamingStatusToolSections.length > 0 ||
      !hasText,
  })
  const hasToolCalls = visibleToolSections.length > 0
  const shouldRenderMessageBubble =
    (isUser
      ? hasText
      : hasRevealedText || (!effectiveIsStreaming && hasText)) ||
    hasAttachments ||
    hasInlineImages ||
  ...
  const artifactSummary = useMemo(() => {
    if (isUser || effectiveIsStreaming) return null
    return deriveArtifactSummary({
      text: assistantDisplayText || fullText,
      attachments,
      toolSections: inlineToolSections,
    })
  }, [
    assistantDisplayText,
    attachments,
  ...
  return (
    ...
    >
      {showStreamingStatus && (
        <StreamingStatusWindow
          activityState={streamingActivityState}
          isStreaming={effectiveIsStreaming}
        />
      )}
      {/* Narration messages (tool-call activity) — compact collapsible row */}
      {!isUser && !showStreamingStatus && (message as any).__isNarration && hasText && (
      ...
      {!showStreamingStatus && hasToolCalls && (
        <ToolCallGroup
          toolSections={visibleToolSections}
      ...
      {shouldRenderMessageBubble && !(message as any).__isNarration && (
        ...
        >
          ...
          <div
            ...
            className={cn(
              'break-words whitespace-normal min-w-0 flex flex-col gap-2 px-3 py-2',
              CHAT_MESSAGE_BUBBLE_WIDTH_CLASS,
              !isUser
          ...
          >
            ...
            {hasText &&
              (isUser ? (
                <MessageContent
                  markdown
                  markdownVariant="inverse"
                  className="bg-transparent w-full text-pretty"
                >
                  {displayText}
                </MessageContent>
              ) : hasRevealedText ? (
      ...
      {artifactSummary ? <ArtifactSummaryBar summary={artifactSummary} /> : null}
