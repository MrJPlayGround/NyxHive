...
export function useDeleteSession(): DeleteSessionResult {
  ...
  const mutation = useMutation({
    ...
    onMutate: async function onMutate(payload) {
      setError(null)
      markSessionDeletedIds(payload.sessionKey, payload.friendlyId)
      ...
      for (const key of [payload.sessionKey, payload.friendlyId]) {
        if (key) store.clearSession(key)
      }
      store.clearSessionWaitingForKeys([payload.sessionKey, payload.friendlyId])
    ...
    onError: function onError(err, _payload, context) {
      ...
      clearSessionDeletedIds(_payload.sessionKey, _payload.friendlyId)
