...
const PORTABLE_HISTORY_STORAGE_KEY = 'nyx_portable_chat_main'
const LEGACY_PORTABLE_HISTORY_STORAGE_KEY = 'hermes_portable_chat_main'
...
function readPortableHistory(): HistoryResponse {
  ...
  try {
    const raw =
      window.localStorage.getItem(PORTABLE_HISTORY_STORAGE_KEY) ||
      window.localStorage.getItem(LEGACY_PORTABLE_HISTORY_STORAGE_KEY)
...
export function useChatHistory({
  ...
  const historyQuery = useQuery({
    ...
    initialData: function useInitialHistory(): HistoryResponse | undefined {
      ...
      if (isNewChat) {
        return { sessionKey: 'new', messages: [] }
      }
  ...
  const rawHistoryMessages = useMemo(() => {
    if (isNewChat && !portableMode) return []
    return Array.isArray(historyQuery.data?.messages)
      ? historyQuery.data.messages
      : []
  }, [historyQuery.data?.messages, isNewChat, portableMode])
  ...
  const resolvedSessionKey = useMemo(() => {
    if (isNewChat && !portableMode) return 'new'
