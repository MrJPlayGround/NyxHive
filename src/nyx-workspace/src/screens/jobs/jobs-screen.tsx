...
import type { NyxJob } from '@/lib/jobs-api'
...
const QUERY_KEY = ['nyx', 'jobs'] as const
...
function getLastRunStatus(job: NyxJob): {
  label: string
  color: string
} {
  if (!job.last_run_at) {
...
function JobCard({
  ...
  const outputQuery = useQuery({
    queryKey: ['nyx', 'jobs', job.id, 'output'],
...
export function JobsScreen() {
  ...
  const searchState = useSearch({ from: '/jobs' })
  const [search, setSearch] = useState('')
  const [showCreate, setShowCreate] = useState(false)
  const [editingJob, setEditingJob] = useState<NyxJob | null>(null)
  const agentFilter = typeof searchState.agent === 'string' ? searchState.agent : null
  ...
  const filteredJobs = useMemo(() => {
    ...
    const jobsForAgent = agentFilter
      ? jobs.filter((job) => job.agent === agentFilter)
      : jobs
    if (!search.trim()) return jobsForAgent
    const q = search.toLowerCase()
    return jobsForAgent.filter(
      (j) =>
        j.name?.toLowerCase().includes(q) ||
        j.prompt?.toLowerCase().includes(q) ||
        j.agent?.toLowerCase().includes(q),
    )
  }, [agentFilter, jobsQuery.data, search])
    ...
    <div className="min-h-full overflow-y-auto bg-surface text-ink">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {agentFilter ? (
            <span className="rounded-full border border-[var(--theme-border)] px-2 py-0.5 text-[10px] uppercase tracking-[0.14em] text-[var(--theme-muted)]">
              {agentFilter}
            </span>
          ) : null}
          {jobsQuery.data && (
