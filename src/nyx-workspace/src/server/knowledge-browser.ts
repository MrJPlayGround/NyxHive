...
class GitHubKnowledgeProvider {
  ...
  constructor(
    ...
    this.branch = branch
    const base = path.join(getNyxWorkspaceHome(), 'knowledge-cache', 'github', safeRepo, branch, safePath)
    this.cacheDir = base
  }

  private get cacheRoot(): string {
    return path.join(getNyxWorkspaceHome(), 'knowledge-cache', 'github', this.repo.replace('/', '_'), this.branch)
  }
  ...
  private async fetchDir(dirPath: string): Promise<void> {
    ...
    const res = await fetch(url, {
      headers: { Accept: 'application/vnd.github.v3+json', 'User-Agent': 'nyx-workspace' },
    })
    if (!res.ok) {
...
export function getEffectiveKnowledgeRoot(): string {
