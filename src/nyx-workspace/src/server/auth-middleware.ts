...
export function isPasswordProtectionEnabled(): boolean {
  return Boolean(
    (process.env.NYX_WORKSPACE_PASSWORD || process.env.HERMES_PASSWORD || '')
      .length > 0,
  )
}
...
export function verifyPassword(password: string): boolean {
  const configured = process.env.NYX_WORKSPACE_PASSWORD || process.env.HERMES_PASSWORD
...
export function getSessionTokenFromCookie(
  ...
  for (const cookie of cookies) {
    if (cookie.startsWith('nyx-auth=')) {
      return cookie.substring('nyx-auth='.length)
    }
    if (cookie.startsWith('nyx-auth=')) {
      return cookie.substring('nyx-auth='.length)
    }
...
export function createSessionCookie(token: string): string {
  ...
  // maxAge: 30 days
  return `nyx-auth=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${30 * 24 * 60 * 60}`
}
