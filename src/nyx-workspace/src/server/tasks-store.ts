...
const TASKS_HOME = path.join(getNyxWorkspaceHome(), 'tasks')
...
export function createReminder(input: CreateReminderInput): TaskRecord {
  const timezone = input.timezone?.trim() || 'Europe/Lisbon'
  const due = new Date(input.remindAt)
  if (Number.isNaN(due.getTime())) {
    throw new Error('Invalid reminder time')
  }
  return createTask({
    title: input.title,
    description: input.description ?? `Reminder scheduled for ${due.toISOString()} (${timezone})`,
    column: 'todo',
    priority: 'medium',
    assignee: 'jay',
    tags: ['reminder', `tz:${timezone}`],
    due_date: due.toISOString(),
    created_by: input.createdBy ?? 'nyx',
  })
}

export function listReminders(includeDone = false): TaskRecord[] {
  return listTasks({ includeDone }).filter((task) => task.tags.includes('reminder'))
}

export function completeReminder(taskId: string): TaskRecord | null {
  const task = getTask(taskId)
  if (!task || !task.tags.includes('reminder')) return null
  return updateTask(taskId, { column: 'done' })
}
