import { streamChat } from './nyx-api'
...
async function* streamNyxChat(
  messages: Array<ChatMessage>,
  options: UnifiedChatOptions,
): AsyncGenerator<string, void, void> {
  if (!options.sessionId) {
    throw new Error('Nyx enhanced chat requires sessionId')
  }
  ...
  if (!lastUserMessage) {
    throw new Error('Nyx enhanced chat requires a user message')
  }
...
export async function sendChatUnified(
  ...
  if (backend === 'nyx-enhanced') {
    let text = ''
    for await (const delta of streamNyxChat(messages, options)) {
      text += delta
    }
    return text
  }
...
export async function streamChatUnified(
  ...
  if (backend === 'nyx-enhanced') {
    return streamNyxChat(messages, options)
  }
