...
const _streamingPersistTimers = new Map<string, ReturnType<typeof setTimeout>>()
const STREAMING_RESTORE_TTL_MS = 10 * 60_000
...
type ChatState = {
  ...
  restoreStreamingSession: (sessionKey: string) => boolean
  hydrateStreamingSession: (sessionKey: string, state: StreamingState) => void
  ...
  /** Clear waiting state for all candidate keys tied to one terminal run */
  clearSessionWaitingForKeys: (
...
function persistStreamingState(
  ...
  const existingTimer = _streamingPersistTimers.get(sessionKey)
  if (existingTimer) clearTimeout(existingTimer)
  const nextTimer = setTimeout(() => {
    sessionStorage.setItem(
      `nyx_streaming_${sessionKey}`,
      JSON.stringify({ ...state, _savedAt: Date.now() }),
    )
    _streamingPersistTimers.delete(sessionKey)
  }, 500)
  _streamingPersistTimers.set(sessionKey, nextTimer)
}
...
export function restoreStreamingState(
  ...
  const storageKey = `nyx_streaming_${sessionKey}`
  ...
  try {
    ...
    if (!savedAt || Date.now() - savedAt > STREAMING_RESTORE_TTL_MS) {
...
const WAITING_STORAGE_PREFIX = 'nyx_waiting_'
...
function stripFinalTags(text: string): string {
  ...
  result = sanitizeAssistantResponse(stripInternalTags(result))
  return result
}
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  clearSessionWaitingForKeys: (sessionKeys) => {
    ...
    const nextKeys = new Set(get().waitingSessionKeys)
    const nextMeta = { ...get().waitingSessionMeta }
    let changed = false

    for (const key of keysToClear) {
      removeWaitingState(key)
      if (nextKeys.delete(key)) changed = true
  ...
  isSessionWaiting: (sessionKey) => {
    if (!get().waitingSessionKeys.has(sessionKey)) return false
    const meta = get().waitingSessionMeta[sessionKey]
    if (meta && Date.now() - meta.since >= WAITING_TTL_MS) {
    ...
    switch (event.type) {
      ...
      case 'user_message': {
        ...
        const duplicateIndex = sessionMessages.findIndex((existing) => {
          ...
          // (e.g. same reply routed from Telegram + Nyx Workspace).
          if (
      ...
      case 'thinking': {
        const streamingMap = new Map(state.streamingState)
        const prev = streamingMap.get(sessionKey) ?? createEmptyStreamingState()
        const lifecycleEvent = parseLifecycleEvent(event.text, now)
        const next: StreamingState = {
          ...prev,
          thinking: '',
          runId: event.runId ?? prev.runId,
          lifecycleEvents: [...prev.lifecycleEvents, lifecycleEvent],
        }
      ...
      case 'done': {
        ...
        if (typeof sessionStorage !== 'undefined') {
          sessionStorage.removeItem(`nyx_streaming_${sessionKey}`)
        }
        get().clearSessionWaiting(sessionKey)
        break
      }
  ...
  restoreStreamingSession: (sessionKey) => {
    const restored = restoreStreamingState(sessionKey)
    if (!restored) return false

    const streaming = new Map(get().streamingState)
    streaming.set(sessionKey, restored)
    set({ streamingState: streaming, lastEventAt: Date.now() })
    return true
  },

  hydrateStreamingSession: (sessionKey, state) => {
    const streaming = new Map(get().streamingState)
    streaming.set(sessionKey, state)
    set({ streamingState: streaming, lastEventAt: Date.now() })
  ...
  clearSession: (sessionKey) => {
    ...
    const streaming = new Map(get().streamingState)
    const waitingKeys = new Set(get().waitingSessionKeys)
    const { [sessionKey]: _, ...waitingMeta } = get().waitingSessionMeta
    messages.delete(sessionKey)
    streaming.delete(sessionKey)
    waitingKeys.delete(sessionKey)
    removeWaitingState(sessionKey)
    if (typeof sessionStorage !== 'undefined') {
      sessionStorage.removeItem(`nyx_streaming_${sessionKey}`)
    }
