...
import { NyxReconnectBanner } from '@/components/nyx-reconnect-banner'
import { useMobileKeyboard } from '@/hooks/use-mobile-keyboard'
import { ErrorBoundary } from '@/components/error-boundary'
// System metrics footer removed — not used in Nyx Workspace
...
const ChatSidebar = lazy(async () => {
  const module = await import('@/screens/chat/components/chat-sidebar')
  return { default: module.ChatSidebar }
})

const ChatPanel = lazy(async () => {
  const module = await import('@/components/chat-panel')
  return { default: module.ChatPanel }
})

const ChatPanelToggle = lazy(async () => {
  const module = await import('@/components/chat-panel-toggle')
  return { default: module.ChatPanelToggle }
})

const MobileHamburgerMenu = lazy(async () => {
  const module = await import('@/components/mobile-hamburger-menu')
  return { default: module.MobileHamburgerMenu }
})
...
const NyxOnboarding = lazy(async () => {
  const module = await import('@/components/onboarding/nyx-onboarding')
  return { default: module.NyxOnboarding }
})

const InspectorPanel = lazy(async () => {
  const module = await import('@/components/inspector/inspector-panel')
  return { default: module.InspectorPanel }
})
...
export function buildActiveRuntimeSessionKeys(
  streamingSessionKeysToken: string,
  waitingSessionKeysToken: string,
): Set<string> {
  const keys = new Set<string>()
  for (const token of [
    ...streamingSessionKeysToken.split('\n'),
    ...waitingSessionKeysToken.split('\n'),
  ]) {
...
export function WorkspaceShell() {
  ...
  const streamingSessionKeysToken = useChatStore((state) =>
    Array.from(state.streamingState.keys()).sort().join('\n'),
  )
  const waitingSessionKeysToken = useChatStore((state) =>
    Array.from(state.waitingSessionKeys).sort().join('\n'),
  )
  const activeRuntimeSessionKeys = useMemo(
    () =>
      buildActiveRuntimeSessionKeys(
        streamingSessionKeysToken,
        waitingSessionKeysToken,
      ),
    [streamingSessionKeysToken, waitingSessionKeysToken],
  )

  const activeRuntimeSession = useMemo(() => {
    if (activeRuntimeSessionKeys.size === 0) return null
    return (
      sessions.find(
        (session) =>
  ...
  }, [activeRuntimeSessionKeys, sessions])

  const activeRuntimeTarget =
    activeRuntimeSession?.friendlyId || Array.from(activeRuntimeSessionKeys)[0]
  ...
  return (
    <>
      <div
        ...
        style={shellStyle}
      >
        <NyxReconnectBanner enabled={authState.checked} />
        ...
        >
          ...
          {!isMobile && !hideChatSidebar && (
            <div className="relative z-30">
              <Suspense fallback={<div className="h-full w-[72px] border-r border-primary-200" />}>
                <ChatSidebar
                  sessions={sessions}
        ...
        {/* Active run return pill — visible on non-chat routes */}
        {!isOnChatRoute && !isMobile && activeRuntimeTarget ? (
          <button
            type="button"
            onClick={() =>
              void navigate({
                to: '/chat/$sessionKey',
                params: { sessionKey: activeRuntimeTarget },
              })
            }
            className="fixed bottom-20 right-6 z-50 rounded-lg border border-accent-500/30 bg-[var(--theme-card)] px-3 py-2 text-left text-xs shadow-lg transition hover:border-accent-500"
            style={{ color: 'var(--theme-text)' }}
          >
            <span className="block font-semibold text-accent-500">
              {activeRuntimeSessionKeys.size > 1
                ? `${activeRuntimeSessionKeys.size} sessions working`
                : `${WORKSPACE_DISPLAY_NAME} is working`}
            </span>
            <span
              className="mt-0.5 block max-w-[220px] truncate"
