...
const STORAGE_KEY = 'nyx-usage-meter-alerts'
const LEGACY_STORAGE_KEY = 'hermes-usage-meter-alerts'
const STATS_VIEW_STORAGE_KEY = 'nyx-stats-view'
const LEGACY_STATS_VIEW_STORAGE_KEY = 'hermes-stats-view'
...
const PREFERRED_PROVIDER_KEY = 'nyx-preferred-provider'
const LEGACY_PREFERRED_PROVIDER_KEY = 'hermes-preferred-provider'
...
function getStoredPreferredProvider(): string | null {
  ...
  try {
    return (
      ...
      window.localStorage.getItem(LEGACY_PREFERRED_PROVIDER_KEY)
    )
  } catch {
    return null
  }
...
function savePreferredProvider(provider: string) {
  ...
  try {
    ...
    window.localStorage.removeItem(LEGACY_PREFERRED_PROVIDER_KEY)
  } catch {
    /* ignore */
  }
}
...
function getStoredStatsView(): StatsView {
  ...
  try {
    const stored =
      window.localStorage.getItem(STATS_VIEW_STORAGE_KEY) ||
      window.localStorage.getItem(LEGACY_STATS_VIEW_STORAGE_KEY)
...
function saveStatsView(view: StatsView) {
  ...
  try {
    ...
    window.localStorage.removeItem(LEGACY_STATS_VIEW_STORAGE_KEY)
  } catch {
    /* ignore */
  }
}
