...
const STORAGE_KEY_SEEN = 'nyx-mobile-setup-seen'
const LEGACY_STORAGE_KEY_SEEN = 'hermes-mobile-setup-seen'
...
export function MobileSetupModal({ isOpen, onClose }: MobileSetupModalProps) {
  ...
  const steps = [
    {
      ...
      body: 'Install Tailscale on the machine running Nyx Workspace, then sign in.',
    ...
    },
    {
      ...
      body: 'Nyx Workspace can talk to any OpenAI-compatible backend on mobile too. Make sure both the workspace and backend stay reachable over Tailscale or your local network.',
      ...
      action: (
        <div className="rounded-lg border border-primary-700 bg-primary-950 px-4 py-3 text-sm text-primary-200">
          Enhanced Nyx gateway APIs are optional. If core chat already works
    ...
    {
      title: 'Open Nyx Workspace on your phone',
      body:
  ...
  const handleNext = () => {
    ...
    localStorage.removeItem(LEGACY_STORAGE_KEY_SEEN)
    onClose()
  }
