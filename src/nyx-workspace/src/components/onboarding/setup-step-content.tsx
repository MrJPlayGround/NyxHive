  ...
  const checkConnection = useCallback(async () => {
    ...
    try {
      ...
      if (!connected) {
        setLastError(
          data.error === 'server_timeout'
            ? 'Nyx did not respond in time.'
            : 'Nyx is not reachable yet.',
        )
      }
    } catch (error) {
  ...
  return (
    <div className="flex w-full flex-col items-center text-center">
      ...
      <p className="mb-6 max-w-md text-base leading-relaxed text-primary-600">
        {status === 'connected'
          ? 'NyxHive is reachable and ready for a workspace run.'
          : status === 'checking'
            ? 'Checking whether the NyxHive runtime is available...'
            : 'NyxHive is not connected yet.'}
      </p>

      {status === 'disconnected' && (
        <div className="mb-6 w-full rounded-2xl border border-red-200 bg-red-50 p-4 text-left">
          <p className="mb-3 text-sm font-medium text-red-700">
            Make sure the Nyx HTTP API server is enabled:
          </p>
          <div className="space-y-2">
            <div>
              <p className="text-xs font-medium text-red-700 mb-1">
                1. Configure the API server in <code>~/.nyxhive/.env</code>:
              </p>
          ...
          <p className="mt-3 text-xs text-red-700">
            The workspace expects NyxHive's built-in HTTP gateway for sessions,
            models, tools, memory, and runtime status.
          </p>
          {lastError && (
...
export function ModelConfigurationStep({
  ...
  const [config, setConfig] = useState<NyxConfigResponse | null>(null)
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      try {
        const response = await fetch('/api/nyx-config', {
          signal: AbortSignal.timeout(5000),
        })
        if (!response.ok) {
        ...
        const data = (await response.json()) as NyxConfigResponse
        if (!cancelled) {
          setConfig(data)
          setStatus('ready')
        }
  ...
  return (
    <div className="flex w-full flex-col items-center text-center">
      ...
      <p className="mb-6 max-w-md text-base leading-relaxed text-primary-600">
        NyxHive provider settings decide which runtime model answers and which
        capabilities are available in the workspace.
      </p>
      ...
      <div className="mb-6 w-full rounded-2xl border border-primary-200 bg-primary-100/70 p-4 text-left">
        ...
        {status === 'error' && (
          <div className="flex items-start gap-3 text-amber-700">
            ...
            <p className="text-sm">
              ...
              still continue if chat works, then return to settings once the
              gateway exposes configuration.
            </p>
          </div>
        )}
        ...
        {status === 'ready' && !hasModel && (
          <div className="flex items-start gap-3 text-amber-700">
            ...
            />
            <p className="text-sm">
              No model is reported yet. Open provider settings and choose the
              model NyxHive should use for workspace runs.
            </p>
          </div>
        )}
      </div>
