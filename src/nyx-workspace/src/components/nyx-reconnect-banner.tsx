...
type NyxReconnectBannerProps = {
  enabled?: boolean
}

type BannerState = 'hidden' | 'disconnected' | 'connected'

async function probeNyxHealth(): Promise<boolean> {
  // Use the portable-aware connection status endpoint first,
  // which works with both full Nyx and OpenAI-compatible backends.
  try {
  ...
  try {
    const response = await fetch('/api/nyx-proxy/health', {
      cache: 'no-store',
    })
    return response.ok
  } catch {
...
export function NyxReconnectBanner({
  enabled = true,
}: NyxReconnectBannerProps) {
  ...
  useEffect(() => {
    ...
    const runProbe = async (showSpinner: boolean): Promise<boolean> => {
      ...
      const pendingProbe = probeNyxHealth()
        .then((connected) => {
  ...
  async function handleStartAgent(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok || !payload.ok) {
        throw new Error(payload.error || 'Failed to start Nyx')
      }
      ...
      setMessage(
        error instanceof Error ? error.message : 'Failed to start Nyx',
      )
    } finally {
      setIsStarting(false)
      await probeNowRef.current?.(true)
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold">
              {isDisconnected ? 'Nyx not connected' : 'Connected'}
            </p>
            {message ? (
