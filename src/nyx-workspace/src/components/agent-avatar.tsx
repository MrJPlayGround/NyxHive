...
export const AGENT_AVATAR_STORAGE_KEY = 'nyx-loader-preference'
const LEGACY_AGENT_AVATAR_STORAGE_KEY = 'hermes-loader-preference'
const AGENT_AVATAR_EVENT = 'nyx-loader-preference-change'
...
export function readAgentAvatarPreference(): AgentAvatarPreference {
  ...
  try {
    const stored =
      window.localStorage.getItem(AGENT_AVATAR_STORAGE_KEY) ||
      window.localStorage.getItem(LEGACY_AGENT_AVATAR_STORAGE_KEY)
...
export function writeAgentAvatarPreference(
  ...
  try {
    ...
    window.localStorage.removeItem(LEGACY_AGENT_AVATAR_STORAGE_KEY)
  } catch {
    // Ignore storage errors
  }
...
export function subscribeToAgentAvatarPreference(
  ...
  function handleStorage(event: StorageEvent) {
    if (
      event.key === AGENT_AVATAR_STORAGE_KEY ||
      event.key === LEGACY_AGENT_AVATAR_STORAGE_KEY
    ) {
      onStoreChange()
    }
  }
