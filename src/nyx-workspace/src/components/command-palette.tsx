...
import { resolveUiSlashCommandAction } from '@/screens/chat/chat-screen-utils'
...
export function CommandPalette({ pathname, sessions }: CommandPaletteProps) {
  ...
  const runSlashCommand = (command: string) => {
    const action = resolveUiSlashCommandAction(command)
  ...
  const slashCommandActions = useMemo<Array<CommandAction>>(
    () => [
      ...
      },
      {
        ...
        group: 'Slash Commands',
        label: '/model',
        keywords: 'open model picker settings nyx provider',
        shortcut: 'Run',
        icon: CommandLineIcon,
